...
create_dropdown(main_frame, "Disable Subscribe Gif:", disable_subscribe_var, ["yes", "no"], 11)
...
Button(
    ...
    command=lambda: scrape_and_process(
        ...
        skip_puppeteer_var.get(),skip_captions_var.get(),disable_subscribe_var.get()
    ),
    width=20, height=2
).grid(row=12, columnspan=2, pady=15)
