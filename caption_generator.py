...
def prepare_file_for_adding_captions_n_headings_thru_html(url, input_video_path="composed_video.mp4", base_file_name="output", language="english", story_text="",description ="", tags="",playlist="",channel="",title="", schedule_date="", shorts_html="",  skip_captions="no", notebooklm="no"):
    ...
    if skip_captions == "yes":
        ...
        return
    if notebooklm == "yes":
        print("For Notbooklm skipping captions generation, storing record in excel.")
        save_details_in_excel("", url, base_file_name, description, tags, playlist, channel,title,schedule_date)
        return
